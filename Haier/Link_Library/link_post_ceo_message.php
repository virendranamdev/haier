...
//echo $path;
            $path_name = $maxid . "-" . str_replace(' ', '', $path);
...
//echo $fullpath;
        ...
        $POST_IMG = ($_FILES['uploadimage']['error'] != 4) ? $folder . $path_name : "";
        $POST_TEASER = $_POST['leadername'];
        $POST_CONTENT = $_POST['content'];
        $POST_IMG_THUMB = ($_FILES['uploadimage']['error'] != 4) ? $thumb_img : "";
        $DATE = $post_date;
        $FLAG = $flag_value;
        ...
        $result = $obj->create_Post($clientid, $POST_ID, $POST_TITLE, $POST_IMG, $POST_IMG_THUMB, $POST_TEASER, $POST_CONTENT, $DATE, $USERID, $BY, $FLAG, $like, $comment, $video_url);
...
// print_r($rt);
                ...
                echo "<script>alert('Post Not Send');</script>";
// print_r($rt);
                echo "<script>window.location='../create_ceo_message.php'</script>";
            }
        } else {
