...
if(empty($publishdate) || $publishdate == "")
{
$ptime1 = $post_date;
}
if(empty($unpublishdate) || $unpublishdate == "")
{
...
else
{
  //  echo "<script>alert('file not created')</script>";
   ...
   /** ******************************* Get GoogleAPIKey and IOSPEM file ********************************* */
		...
		$userimage = $push_obj->getImage($createdby);
        $image = $userimage[0]['userImage'];
		
		/*********************** end user image ****************/
		
   $result = $obj1->addNotice($client,$maxid,$title,$pagename,$content,$createdby,$ptime1,$utime1,$post_date, $FLAG, $device);
   //print_r($result);
   $type = 'Notice';
   $img = "";
...
//echo $result1;
/***********************  custom group *********************/
         $groupdetails = $read->getGroupDetails($client, $myArray[$k]);  //get all groupdetails
...
/******************  fetch all user employee id from user detail start *****************************/
  if (count($general_group) > 0) {
        ...
        $gcm_value = $push_obj->get_Employee_details($User_Type, $general_group, $client);
    ...
    if (count($custom_group) > 0) {
        $gcm_value1 = $customGroup->getCustomGroupUser($client, $custom_group);
...
//print_r($data);
 ...
 $IOSrevert = $push_obj->sendAPNSPush($data, $idsIOS, $googleapiIOSPem['iosPemfile']);
  ...
  $rt = json_decode($revert, true);
	 ...
	 if($rt['success'] == 1)
	 {
	 echo "<script>alert('Communique Successfully Created');</script>";
