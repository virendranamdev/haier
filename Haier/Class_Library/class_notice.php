...
class Notice {
    ...
    function addNotice($client, $maxid, $title, $noticename,$content,$createdby, $ptime, $utime, $post_date, $flag, $device) {
        $this->id = $maxid;
        $this->title = $title;
        ...
        //echo $postid;
        try {
            $qu = "insert into Tbl_C_NoticeDetails(ClientId,noticeId,noticeTitle,fileName,noticeData,createdBy,status,device,flagType,publishingTime,unpublishingTime,createdDate)
                values(:cid,:nid,:ntitle,:fname,:ndata,:cb,:st,:device,:flagt,:pt,:upt,:cd)";
            ...
            $stmt->bindParam(':fname', $this->noticename, PDO::PARAM_STR);
             $stmt->bindParam(':ndata',$content, PDO::PARAM_STR);
...
//        $path = "admin.benepik.com/employee/virendra/Mahle_AdminPanel/";
    ...
    function updateNotice($clientid, $noticeid, $title,$noticecontent) {
        try {
            $qu = "update Tbl_C_NoticeDetails set noticeTitle=:ntitle,noticeData=:ncontent where clientId=:cli and noticeId=:nid";
            $stmt = $this->DB->prepare($qu);
            $stmt->bindParam(':nid', $noticeid, PDO::PARAM_STR);
            $stmt->bindParam(':ntitle', $title, PDO::PARAM_STR);
             $stmt->bindParam(':ncontent', $noticecontent, PDO::PARAM_STR);
