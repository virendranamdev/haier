        ...
        try {
            ...
            if (count($rows) > 0) {
                ...
                if ($count_group <= 0) {
                    ...
                    if ($module == 1) {
                        $noticequery = "select distinct(emplearningId) as emplearningId from Tbl_Analytic_LearningSentToGroup where groupId IN('" . $in . "') 
							 and clientId =:cid order by autoId desc limit 5";
                    } else {
                        $noticequery = "select distinct(emplearningId) as emplearningId from Tbl_Analytic_LearningSentToGroup where groupId IN('" . $in . "') 
							 and clientId =:cid order by autoId desc ";
//                        . limit $this->value . ",5";
                    }
