...
//$val["UserName"]=$rows["firstName"];
                         ...
                         /*   case "CEOMessage":
                                $eventquery1 = "select *,Concat('CEOMessage','') as type, DATE_FORMAT(created_date,'%d %b %Y') as created_date, if(post_img IS NULL or post_img='' ,'', Concat('$site_url',post_img)) as post_img from Tbl_C_PostDetails where Post_id =:id and clientId =:cid and flagCheck = 9 and status = 'Publish'";
                                $nstmt = $this->DB->prepare($eventquery1);
                                $nstmt->bindParam(':cid', $this->idclient, PDO::PARAM_STR);
                                $nstmt->bindParam(':id', $welid, PDO::PARAM_STR);
                                $nstmt->execute();
...
*/
    ...
    postId =:id and clientId =:cid and status = 1";
                                $nstmt = $this->DB->prepare($noticequery);
                                $nstmt->bindParam(':cid', $this->idclient, PDO::PARAM_STR);
                                $nstmt->bindParam(':id', $welid, PDO::PARAM_STR);
                                $nstmt->execute();
             ...
             if(post_img IS NULL or post_img='' , '', Concat('$site_url',post_img)) as post_img, DATE_FORMAT(created_date,'%d %b %Y') as created_date from Tbl_C_PostDetails where post_id=:id and clientId =:cid and status = 'Publish'";
                                    $nstmt = $this->DB->prepare($noticequery1);
                                    $nstmt->bindParam(':cid', $this->idclient, PDO::PARAM_STR);
                                    $nstmt->bindParam(':id', $welid, PDO::PARAM_STR);
                                    $nstmt->execute();
