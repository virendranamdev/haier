...
require_once('connect_analysis_PDO.php');
...
class Category {
    ...
    public function __construct() {
        ...
        $dbh1 = new analysisConnection(/* ... */);
        $this->db_analysis = $dbh1->getConnectionAnalysis();
    }

    function categoryDisplay($clientid, $employeeid) {

        try {
            $query = "select d_category.* from d_category join B_Client_Data join UserDetails where d_category.dc_id!=B_Client_Data.categoryHide and B_Client_Data.client_id=:cli and UserDetails.clientId=:cli and UserDetails.employeeId=:empid";
...
//$query_params2 = array();
    ...
    function entryShowDeals($clientid, $employeeID, $catname, $device) {

        date_default_timezone_set('Asia/Calcutta');
        $cat_date = date('d M Y, h:i A');

        try {
            $query = "insert into showdealsAnalysis(employeeID,clientId,categoryName,device,dateEntry) values(:empid,:packnam,:cat,:dev,:dat)";
            $stmt = $this->db_analysis->prepare($query);
            $stmt->bindParam(':empid', $employeeID, PDO::PARAM_STR);
            $stmt->bindParam(':packnam', $clientid, PDO::PARAM_STR);
            $stmt->bindParam(':cat', $catname, PDO::PARAM_STR);
            $stmt->bindParam(':dev', $device, PDO::PARAM_STR);
            $stmt->bindParam(':dat', $cat_date, PDO::PARAM_STR);
            $stmt->execute();
        } catch (PDOException $e) {
    ...
    function categoryShowDealsByValue($clientid, $employeeid, $categoryid, $location) {
        if ($location != "") {
            $query = "SELECT deal.deal_id,deal.category,deal.img_logo,deal.percentage_offer,deal.merchant_name,d_category.AppointText, merchant_branch.* from deal join merchant_branch on deal.deal_id = merchant_branch.deal_id join d_category on deal.category = d_category.dc_category join UserDetails where UserDetails.clientId=:cli and UserDetails.employeeId=:empid and d_category.dc_id=:category and merchant_branch.city=:location and deal.status='active' order by deal.priority";
            $query_params = array(
                ':category' => $categoryid,
                ':location' => $location,
                ':cli' => $clientid,
        ...
        try {
            ...
            if ($branchname == "Buy Online") {
                ...
                if (!empty($value1)) {
                    ...
                    $coupancode = "Sorry !! Currently No Coupon Available";
            }
            else {
...
//execute query
    ...
    function entryDeal($clientid, $employee, $dealNam, $device) {
        ...
        date_default_timezone_set('Asia/Calcutta');
        $deal_date = date('d M Y, h:i A');

        try {
            $query = "insert into dealAnalysis(clientId,employeeId,dealName,device,dateEntry) values(:packnam,:uui,:cat,:dev,:dat)";
            $stmt = $this->db_analysis->prepare($query);
            $stmt->bindParam(':packnam', $clientid, PDO::PARAM_STR);
