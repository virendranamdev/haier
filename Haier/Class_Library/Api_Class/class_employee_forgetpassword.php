...
class ForgotPassword {
    ...
    function forgotPasswordSentTo($packName, $empcode) {
        $username = trim(strtoupper($empcode));
        try {
            $query = "select ud.firstName, ud.middleName, ud.lastName, ud.emailId, ud.contact,ud.employeeId,ud.employeeCode,cd.dedicated_mail,cd.client_id,cd.responseDecider,cd.program_name from Tbl_ClientDetails_Master as cd join Tbl_EmployeeDetails_Master as ud where cd.packageName=:package and cd.client_id= ud.clientId and "
                    . "(UPPER(ud.employeeCode)=:empcode or UPPER(ud.contact)=:empcode or UPPER(ud.emailId)=:empcode)";
            $stmt = $this->db_connect->prepare($query);
            $stmt->bindParam(':empcode', $username, PDO::PARAM_STR);
