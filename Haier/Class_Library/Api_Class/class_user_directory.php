...
class UserDirectory {
    ...
    function getLocationDepartment($cid) {
        ...
        try {
            $query = "select distinct(branch) as location from Tbl_EmployeeDetails_Master where clientId =:id1 order by branch";
    ...
    function viewDepartments($cid, $locid) {
        ...
        try {
           // $query = "select distinct(department) as department from Tbl_EmployeeDetails_Master where clientId =:id1 and location=:loc";
            $query = "select autoId, department from Tbl_EmployeeDetails_Master where clientId =:id1 and branch=:loc group by (department)";
            $stmt = $this->db_connect->prepare($query);
        ...
        try {
            if ($this->department != "") {
                ...
                $query = "select edm.*,epd.*,if(epd.userImage IS NULL or epd.userImage='','', ConCat('$server_name',epd.userImage)) as imgpath from  Tbl_EmployeeDetails_Master  as edm left join Tbl_EmployeePersonalDetails as epd on "
                        . "epd.employeeCode = edm.employeeCode where edm.clientId =:id1 and edm.branch=:loc and edm.department = :dep";
                $stmt = $this->db_connect->prepare($query);
                ...
                $query = "select edm.*,epd.*,if(epd.userImage IS NULL or epd.userImage='','', ConCat('$server_name',epd.userImage)) as imgpath from  Tbl_EmployeeDetails_Master  as edm left join Tbl_EmployeePersonalDetails as epd on "
                        . "epd.employeeCode = edm.employeeCode where edm.clientId =:id1 and edm.branch=:loc";
                $stmt = $this->db_connect->prepare($query);
                $stmt->bindParam(':id1', $this->clientids, PDO::PARAM_STR);
                $stmt->bindParam(':loc', $this->locations, PDO::PARAM_STR);
                $stmt->execute();
