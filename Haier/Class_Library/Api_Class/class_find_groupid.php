...
//echo "final string : - ".$finalstring."<br/>";
    ...
    function groupBaseofemployeeid($clientId, $empcode) {
        try {
            $query = "select distinct(groupId) from Tbl_CustomGroupDetails where clientId = :cid and employeeId = :eid";
            $stmt51 = $this->DB->prepare($query);
            $stmt51->bindParam(':cid', $clientId, PDO::PARAM_STR);
            $stmt51->bindParam(':eid', $empcode, PDO::PARAM_STR);
            $stmt51->execute();
            $ft = $stmt51->fetchAll(PDO::FETCH_ASSOC);
            $selectgroup = array();
            foreach( $ft as $gr)
            {
                array_push($selectgroup, $gr['groupId']);
            }
          //  print_r($selectgroup);
