...
//                    . "limit " . $this->value . " , 5";
    ...
    function getSingleOnboard($postid, $clientid, $device='') {
        $this->id = $postid;
        $this->cli = $clientid;
		if($device=='panel')
		{
     ...
     //   $query = "select post.*, DATE_FORMAT(post.created_date,'%d %b %Y') as created_date , if(post.thumb_post_img IS NULL or post.thumb_post_img='' , CONCAT('" . $site_url . "',post.post_img), CONCAT('" . $site_url . "',post.thumb_post_img)) as post_img, if(user.userImage IS NULL or user.userImage='','',CONCAT('" . $site_url . "',user.userImage)) as userImage from Tbl_C_PostDetails as post join Tbl_EmployeePersonalDetails as user on post.userUniqueId = user.employeeId where post.post_id =:pid and post.clientId=:cli";
        ...
        $query = "select post.*, DATE_FORMAT(post.created_date,'%d %b %Y') as created_date , if(post.post_img IS NULL or post.post_img='','', CONCAT('" . $site_url . "',post.post_img)) as post_img,post.post_img as onboarimg , if(user.userImage IS NULL or user.userImage='','',CONCAT('" . $site_url . "',user.userImage)) as userImage from Tbl_C_PostDetails as post join Tbl_EmployeePersonalDetails as user on post.userUniqueId = user.employeeId where post.post_id =:pid and post.clientId=:cli and post.flagCheck=12";
        try {
            $stmt = $this->DB->prepare($query);
