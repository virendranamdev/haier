...
//print_r($result);
    ...
    $count_group = count($grouparray);
                ...
                if ($count_group <= 0) {
                    $response["success"] = 0;
                    $response["message"] = "No More Notice Available";
                } else {
                 ...
                 //   echo "group array : ".$in."<br/>";
                    ...
                    if ($module == 1) {
                        $noticequery = "select distinct(noticeId) as noticeid from Tbl_Analytic_NoticeSentToGroup where groupId IN('" . $in . "') 
							 and clientId =:cid and status = 1 order by autoId desc limit 5";
                    } else {
                        $noticequery = "select distinct(noticeId) as noticeid from Tbl_Analytic_NoticeSentToGroup where groupId IN('" . $in . "') 
							 and clientId =:cid and status = 1 order by autoId desc limit " . $this->value . ",5";
                    }
                    ...
                    if (count($noticerows1) > 0) 
                        ...
                        foreach ($noticerows1 as $noticerows) 
                            ...
                            $query2 = "select *, DATE_FORMAT(createdDate,'%d %b %Y') as createdDate,DATE_FORMAT(publishingTime,'%d %b %Y') as publishingTime, 
                            DATE_FORMAT(unpublishingTime,'%d %b %Y') as unpublishingTime  
                            ...
                            $post["noticedata"] = $rows["noticeData"];
                        ...
                        $response['success'] = 0;
                        $response['msg'] = "No More Notice Available";
                    }
                }
            } else {
                $response['success'] = 0;
    ...
    function getSingleNotice($noticeid, $clientid, $flag='') {
        $this->id = $noticeid;
        $this->cli = $clientid;
       // $site_url = SITE_URL;
        $site_url = dirname(site_url).'/';
        $query = "select *, if(fileName IS NULL or fileName='','',CONCAT('$site_url',fileName)) as fileName from Tbl_C_NoticeDetails where noticeId =:pid and clientId=:cli";
        if(!empty($flag)){
            $query .= " and flagType = '$flag'";
        }
        try {
            $stmt = $this->DB->prepare($query);
    ...
    function updateNoticeStatus($pid, $sta) {
        ...
        try {
            ...
            if ($stmt->execute()) {
                $response["success"] = 1;
                $response["message"] = "Successfully Communique status changed";
                return json_encode($response);
            }
        } catch (PDOException $e) {
            echo $e;
