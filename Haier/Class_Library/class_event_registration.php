...
class EventRegistration {
    ...
    function createEventRegistration($clientid,  $createdBy, $eventid, $device) {
        ...
        try {
            ...
            if ($stmt->execute()) {
                $res["success"] = 1;
                $res['msg'] = "You successfully registered";
    ...
    function getAllEventRegistration($clientid, $eventid) {
        ...
        try {
            ...
            for ($k = 0; $k < $count; $k++) {
                ...
                echo "uuid :-" . $uuid . "<br/>";
                $cid = $ROW[$k]['clientId'];
                $query1 = "select ud.*, IF(up.linkedIn='1', up.userImage, IF(up.userImage IS NULL or up.userImage='','',CONCAT('" . $imagepath . "',up.userImage)))as userImage from Tbl_EmployeeDetails_Master as ud join Tbl_EmployeePersonalDetails as up on ud.employeeId = up.employeeId where ud.employeeId =:uid and ud.clientId =:cid";
