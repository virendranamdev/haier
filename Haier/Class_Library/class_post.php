...
class Post {
 ...
 // echo  $valueimage."--".$imagevalue;
    ...
    function create_Post($cid, $pid, $ptitle, $pimg, $thumb_img, $teaser, $pcontent, $pdate, $mail, $by, $flag, $like, $comment, $device, $video_url = '') {
        $this->postid = $pid;
        ...
        try {
            $query = "insert into Tbl_C_PostDetails(clientId,post_id,post_title,post_img,thumb_post_img,postTeaser,post_content,created_date,created_by,userUniqueId,flagcheck,likeSetting,comment,status,device,video_url)
            values(:cid,:pid,:pt,:pi,:thumb_img,:teaser,:pc,:cd,:cb,:em,:fl,:lk,:ck,:st,:device,:video_url)";
...
//$cid, $pid, $title, $content, $POST_IMG, $post_date, $USERID
 function updatePost($cid,$pid,$title,$content, $POST_IMG, $post_date, $USERID, $video_url='',$leadername='')
{
	if($leadername=='')
	{
		$teaser='';
	 ...
	 $this->video_url = (!empty($video_url))?$video_url:'';
		...
		try
		{
		 ...
		 $qu = "update Tbl_C_PostDetails set post_title=:ptitle,post_content=:content,post_img =:pimg, updatedDate =:cd,updatedBy=:by,postTeaser=:teaser,video_url=:video_url where post_id =:pid and clientId =:cid";
