...
location,branch,grade,status,accessibility,createdDate,createdBy) values(:uid,:cid,:eid,:fname,:mname,:lname,:gen,:email,:pass,:ecode,:mob,:officeno,:rm,:dep,:des, :loc,:bra,:gra,:sta,:acc,:cred,:creb) ON DUPLICATE KEY UPDATE firstName =:fname,middleName=:mname, lastName=:lname,gender=:gen,emailId=:email,contact=:mob,officeno=:officeno,reportingmanager=:rm, department=:dep,designation=:des,location=:loc,branch=:bra,grade=:gra,status=:sta,createdDate=:cred,createdBy=:creb";
                    ...
                    if ($stmt->execute()) {
			$dob = date('Y-m-d', strtotime($userdata[$row][4]));
			$doj = date('Y-m-d', strtotime($userdata[$row][5]));
                        $query4 = "insert into Tbl_EmployeePersonalDetails(userid,clientId,employeeCode,employeeId,emailId,userDOB,userDOJ,address)
	                                         values(:uid1,:cid1,:ecode1,:eid1,:emailid1,:dob,:doj,:addr)
	      ON DUPLICATE KEY UPDATE userDOB=:dob,userDOJ=:doj,emailId=:emailid1,address=:addr";
...
// we comment this code because password not send during uploading csv
   ...
   ' . "\n\n" .
    ...
    function userForm($clientid1, $user, $fname, $mname, $lname, $emp_code, $dob, $doj, $email_id, $designation, $department, $contact, $location, $branch, $grade, $gender,$officeno,$rmanager,$address) {
        ...
        try {
            $qu = "insert into Tbl_EmployeeDetails_Master
	(userId,clientId,employeeId,firstName,middleName,lastName,gender,emailId,password,employeeCode,contact,department,designation,location,branch,grade,status,accessibility,createdDate,createdBy,officeno,reportingmanager) values(:uid,:cid,:eid,:fname,:mname,:lname,:gen,:email,:pass,:ecode,:con,:dep,:des, :loc,:bra,:gra,:sta,:acc,:cred,:creb,:offno,:rmanagr) ON DUPLICATE KEY UPDATE firstName =:fname,middleName=:mname, lastName=:lname,gender=:gen, emailId=:email, contact=:con,department=:dep,designation=:des,location=:loc,branch=:bra,grade=:gra, accessibility=:acc,createdDate=:cred,createdBy=:creb,officeno = :offno,reportingmanager=:rmanagr";
            ...
            $stmt->bindParam(':cred', $c_date, PDO::PARAM_STR);
            $stmt->bindParam(':creb', $this->createdby, PDO::PARAM_STR);
			$stmt->bindParam(':offno', $this->officeno, PDO::PARAM_STR);
			$stmt->bindParam(':rmanagr', $this->rmanager, PDO::PARAM_STR);
            if ($stmt->execute()) {

                $query4 = "insert into Tbl_EmployeePersonalDetails(userid,clientId,employeeCode,employeeId,emailId,userDOB,userDOJ,address)
				values(:uid1,:cid1,:ecode1,:eid1,:emailid1,:dob,:doj,:address) ON DUPLICATE KEY UPDATE userDOB=:dob,userDOJ=:doj,emailId=:emailid1,address=:address";
                $stmt4 = $this->DB->prepare($query4);
                $stmt4->bindParam(':uid1', $usid, PDO::PARAM_STR);
                $stmt4->bindParam(':cid1', $clientid, PDO::PARAM_STR);
                ...
                $stmt4->bindParam(':doj', $doj, PDO::PARAM_STR);
				$stmt4->bindParam(':address', $this->address, PDO::PARAM_STR);
