...
class User {
    ...
    public function __construct() {
        ...
        $this->DB = $db->getConnection_Mahle();
    }
    ...
    function uploadUserCsv($clientid1, $user, $file_name, $file_temp_name, $fullpath) {
        ...
        if ($imageFileType != "csv") {
            ...
            for ($row = 1; $row < $countrows; $row++) {
				...
				print_r($userdata[$row][4]);
		    echo "DOB is ".$DOB = $userdata[$row][4];
			echo "DOJ is ".$DOj = $userdata[$row][5];
			echo "email id is ".$email = $userdata[$row][12];
			echo "hello";
